...
{
public:
    double k, ka, NuL, Nux, Rex, Pe, Pr, q, RaL, ReL, mu, nu, h, qs, uinf, rho, hx;
    Flow_Parameters()
    {
...
void fin_1D_plate_sample()
{
    ...
    fo(i, 1, n)
    {
        ...
        fo(j, 1, m)
        {
            ...
            double P = 2 * del_y, Ueq = c1.hx * (2 * c1.k / del_y) / (c1.hx + (2 * c1.k / del_y)), msq = c1.hx * 2 * (del_x + del_y) / (del_x * del_y);
            if (i == 1)
            {
                // ab += 2 * as;
                as = 0;
    ...
    for (i = 1; i <= n; i++)
    {
        ...
        for (j = 1; j <= m; j++)
        {
            ...
            if (j == 1 || j == m)
                P += f1.t;
            // MyFile << setprecision(4) << fixed << phi[i][j] << " " << c1.hx * P * del_y * (phi[i][j] - f1.Tinf) << "  ";
