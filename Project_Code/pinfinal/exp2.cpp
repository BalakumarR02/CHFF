...
{
public:
    double A, Tf, Ab, Af, Ap, As, Auf, Atot, P, L1, W1, B, H, D, L, W, Tb, Tinf, N, S, qb, V, S1;
    Pin_Fin_variables()
    {
        Tinf = 293;
        Tb = 373;
    ...
    void change_fin_dim(double L, double H, double N, double D)
    {
        this->L = L;
        this->H = H;
        this->D = D;
...
{
    ofstream PCT("pin_central_temp.txt");
    ofstream PH("pin_heat.txt");
    ofstream PR("pin_Resistance.txt");
    ofstream NOF("pin_nof.txt");
    ...
    ll change_H, change_N, change_t;
    double del_H = 50 * 0.001, del_N = 18, O_N = f1.N, O_H = f1.H, H_1 = f1.H, O_t = f1.D, del_t = 1 * 0.001;
    fo(cl, 1, 5)
    {
        f1.change_fin_dim(f1.L + (cl > 0 ? (100 * 0.001) : 0), O_H, O_N, O_t);
        PH << f1.L << "\n[";
        ...
        fo(change_H, 0, 20)
        {
            ...
            fo(change_N, 1, 1)
            {
                ...
                fo(change_t, 0, 0)
                {
                    ...
                    double t_1 = f1.D + (change_t > 0 ? 1 : 0) * del_t;
                    f1.change_fin_dim(f1.L, f1.H, f1.N, t_1);
                    vector<vector<vector<double>>> v(n + 1, vector<vector<double>>(m + 1, vector<double>(7)));
